...
BlackprintEventFallback.error.types = {
	node_port_not_found({ iface, portName }){
		...
		ErrorDisplay.addError(`Node port not found for ${iface.title} with name: ${portName}`);
	},
	node_not_found({ namespace }){
		SmallNotif.add(`Node for ${namespace} was not found, maybe .registerNode() haven't being called?`, 'red');
		ErrorDisplay.addError(`Node with namespace (${namespace}) was missing or not loaded`);
	},
	...
	node_template_not_found({ tagName, element }){
		...
		ErrorDisplay.addError(`Node with namespace (${namespace}) haven't been registered as component, maybe the interface file (.sf.js) was missing or not loaded`);
	},
};

BlackprintEventFallback.printError = function(error){
	let message = (error.stack ?? error.message);
	return message.replace(/[\w\-]+\:\/\/.*?(?=\/[^\/]*?\.js)/gm, '<somewhere>'); // Trim path
}

Blackprint.on('error', {slot: 'bp-editor'}, function(ev){
	let error = e instanceof ErrorEvent ? e.error : e;
	if(error instanceof Error){
		ErrorDisplay.addError(BlackprintEventFallback.printError(error));
	}
